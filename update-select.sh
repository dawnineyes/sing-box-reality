...
USE_PRERELEASE=1   # 0 = 正式版, 1 = Pre-release
...
if [ "$USE_PRERELEASE" -eq 1 ]; then
  # 获取最新 Pre-release 版本
  VERSION_TAG=$(curl -s https://api.github.com/repos/SagerNet/sing-box/releases \
    | jq -r '[.[] | select(.prerelease == true)][0].tag_name')
else
  # 获取最新正式版
  VERSION_TAG=$(curl -s https://api.github.com/repos/SagerNet/sing-box/releases/latest \
    | jq -r '.tag_name')
fi
...
if [ "$USE_PRERELEASE" -eq 1 ]; then
  echo "📋 最新 Pre-release 版本: $LATEST_VERSION"
else
  echo "📋 最新正式版: $LATEST_VERSION"
fi
